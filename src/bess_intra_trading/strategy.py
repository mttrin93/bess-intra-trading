...
# Absolute import for the optimization model
from bess_intra_trading.model import solve_intrinsic_problem
# Assuming connect_db is not needed here, but the data loader is
# from bess_intra_trading.data import load_market_data
...
class RollingIntrinsicStrategy:
    ...
    def __init__(self, bess_params: dict, horizon_h: float = 1.0):
        ...
        self.dt = self.params.get('time_step_h', 0.25)  # Default 15 min step

    def simulate(self, market_data: pd.DataFrame, initial_soc: float) -> pd.DataFrame:
        """
        Runs the rolling simulation over the market data.

        Args:
            market_data (pd.DataFrame): Preprocessed data with 'bid' and 'ask' columns.
        ...
        T_horizon = int(self.horizon_h / self.dt)  # Number of time steps in the horizon
